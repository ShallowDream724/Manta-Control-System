...
1. **三端适配设计** - 手机、平板、电脑统一界面，响应式布局
2. **可折叠结构** - 循环、延时等容器支持折叠/展开
3. **清晰的操作逻辑** - 用户能轻松理解串行/并行关系
4. **动态动作生成** - 根据设备配置动态生成可用动作
5. **严格的嵌套限制** - 前端不提供在子步骤中添加循环的选项
6. **实时验证** - 编辑过程中实时验证配置合理性
7. **无Emoji设计** - 使用图标字体和SVG，避免低质感emoji
